...
| 1-3 | 異常系：自動募集未登録 | 対象ギルドに自動募集が未登録 | `AppError::Business`が返る |
...
| 2-4 | 異常系：自動募集未登録 | 対象ギルドに自動募集が未登録 | `AppError::Business`が返る |
...
| 3-3 | 異常系：自動募集未登録 | 対象ギルドに自動募集が未登録 | `AppError::Business`が返る |
...
| 6-2 | 異常系：days範囲外（1以下） | days=1 | `AppError::Business`が返る |
| 6-3 | 異常系：days範囲外（8以上） | days=8 | `AppError::Business`が返る |
| 6-4 | 異常系：既に登録済み | 同一ギルドで自動募集が登録済み | `AppError::Business`が返る |
| 6-5 | 異常系：チャンネル作成失敗時のロールバック | `MockDiscordGateway::create_channel`を失敗させる | `AppError::ChannelCreationFailed`が返り、`auto_recruitments`/関連チャンネルレコードが作成されない |
...
| 7-3 | 異常系：未登録ギルドでの解除 | 自動募集が未登録 | `AppError::Business`が返る |
| 7-4 | 準正常系：Discord削除失敗を含む解除 | `delete_channel`/`delete_message`の一部を失敗させる | エラーをログ出力しつつ`Ok(())`で完了し、DBレコードは削除される |
...
| 8-3 | 異常系：同じ日数への変更 | 現在3日→new_days=3 | `AppError::Business`が返る |
| 8-4 | 異常系：範囲外の日数 | new_days=1 or 8 | `AppError::Business`が返る |
| 8-5 | 異常系：日数増加時のチャンネル作成失敗でロールバック | 現在3日、new_days=5、途中の`create_channel`を失敗させる | `AppError::ChannelCreationFailed`が返り、`days_range`と日時チャンネル構成は変更前のまま |
