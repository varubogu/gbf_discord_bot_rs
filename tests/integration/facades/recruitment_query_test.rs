...
async fn test_search_quests_for_autocomplete() {
    ...
    // 空文字で全クエスト検索
    let options = quest_list::search_quests_for_autocomplete(conn, 0, "").await;
...
async fn test_search_quests_no_match() {
    ...
    let options =
        quest_list::search_quests_for_autocomplete(conn, 0, "存在しないクエスト名XXXXX").await;
...
async fn test_list_quests_for_select() {
    ...
    let quests = quest_list::list_quests_for_select_with_db(conn).await;
...
async fn test_list_battle_styles_for_select() {
    ...
    let styles = battle_style_list::list_battle_styles_for_select_with_db(conn).await;
...
async fn test_get_battle_style_name_by_invalid_id() {
    ...
    let name = battle_style_list::get_battle_style_name_by_id_with_db(conn, 99999).await;
