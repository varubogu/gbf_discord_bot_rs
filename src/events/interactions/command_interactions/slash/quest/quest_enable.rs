...
async fn quest_name_autocomplete(ctx: PoiseContext<'_>, partial: &str) -> Vec<AutocompleteChoice> {
    ...
    let results = quest_management_facade::search_disabled_quests_for_autocomplete(
        &ctx.data().app_state,
        guild_id,
        partial,
    )
...
pub async fn quest_enable(
    ...
    let result = quest_management_facade::change_quest_state(
        ...
        guild_id,
        quest_names,
        quest_management_facade::QuestStateChangeAction::Enable,
    )
    .await?;
    let success_count = result.changed_count;
    let already_enabled = result.already_in_target_state;
