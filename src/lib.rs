...
pub mod test_utils {
    ...
    // DB接続を伴うテストは testcontainers 上の使い捨てPostgreSQLで実行する
    // （`tests/integration/facades/test_helper.rs`）。
    // 以前ここにあった環境変数ベースの接続可否判定ユーティリティは、
    // DB未設定時にテストが黙ってスキップされ「成功」と表示される原因になっていたため削除した。
