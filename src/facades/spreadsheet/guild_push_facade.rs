...
pub async fn execute_push(ctx: &PoiseContext<'_>) -> Result<()> {
    ...
    if let Err(permission_error) = has_permission_result {
        ctx.say(&format!("権限エラー: {}", permission_error)).await?;
        return Ok(());
    }
    ...
    info!("User {} started server-specific spreadsheet push in guild {}", 
    ...
    match result {
        Ok(_) => {
            txn.commit().await?;
            ctx.say("データベースからサーバー固有スプレッドシートへ書き込み完了").await?;
        ...
        Err(e) => {
            ...
            ctx.say("データベースからサーバー固有スプレッドシートへ書き込み失敗").await?;
            Err(e)
        }
    }
}
