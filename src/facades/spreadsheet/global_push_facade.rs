...
pub async fn execute_global_push(ctx: &PoiseContext<'_>) -> Result<()> {
    ...
    info!("User {} started global spreadsheet push in admin server", ctx.author().id);
    ...
    match result {
        Ok(_) => {
            txn.commit().await?;
            ctx.say("データベースからグローバルスプレッドシートへ書き込み完了").await?;
        ...
        Err(e) => {
            ...
            ctx.say("データベースからグローバルスプレッドシートへ書き込み失敗").await?;
            Err(e)
        }
    }
}
