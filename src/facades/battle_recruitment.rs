...
pub(crate) async fn new(ctx: &PoiseContext<'_>, quest_alias: &str, battle_type: BattleType) -> Result<(), String> {
    ...
    // トランザクション内で処理を実行
    match tx_manager.execute_in_transaction(|_tx_ctx| Box::pin(async move {
...
pub(crate) async fn information_update(ctx: &poise::serenity_prelude::Context, guild_id: u64, channel_id: u64, message_id: u64, new_content: Option<String>) -> Result<(), String> {
    ...
    // トランザクション内で処理を実行
    match tx_manager.execute_in_transaction(|tx_ctx| Box::pin(async move {
        ...
        // クエストと日時からメッセージを作成
        service.update_recruitment_message(&serenity_ctx, guild_id, channel_id, message_id, new_content, None).await?;
...
pub(crate) async fn member_update(ctx: &poise::serenity_prelude::Context, guild_id: u64, channel_id: u64, message_id: u64) -> Result<(), String> {
    ...
    match tx_manager.execute_in_transaction(|tx_ctx| Box::pin(async move {
        ...
        // 募集情報をDBから取得
        let recruitment = service.get_recruitment_from_db(guild_id, channel_id, message_id).await?;

        // 募集メッセージのリアクションとメンバーを取得
        let participants_by_reaction = service.get_reactions_and_members(&serenity_ctx, channel_id, message_id).await?;

        // リアクションとメンバーからメッセージを作成
        // クエストと日時からメッセージを作成
        let content = format!("募集ID: {} の参加者を更新しました", recruitment.id);

        service.update_message(&serenity_ctx, channel_id, message_id, &content, &participants_by_reaction).await?;
...
pub(crate) async fn cancel(ctx: &PoiseContext<'_>, guild_id: u64, channel_id: u64, message_id: u64) -> Result<(), String> {
...
pub(crate) async fn start(ctx: &poise::serenity_prelude::Context, guild_id: u64, channel_id: u64, message_id: u64, recruitment_id: i64) -> Result<(), String> {
    ...
    match tx_manager.execute_in_transaction(|tx_ctx| Box::pin(async move {
        ...
        // 開始メッセージを作成
        let start_message = service.create_start_message("サンプルクエスト", &participants).await?;
        
        // 開始返信送信
        service.send_start_reply(&serenity_ctx, channel_id, message_id, &start_message).await?;
        
        // 募集を開始済み状態に更新
        service.mark_recruitment_as_started(recruitment_id, message_id).await?;
