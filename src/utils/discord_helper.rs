...
pub async fn get_reaction_users(
    ...
    for reaction in &message.reactions {
        ...
        let reaction_users = match message
...
pub async fn update_embed_with_participants(
    ...
    for (reaction, users) in &reactions {
        ...
        embed
            .fields
            .push(poise::serenity_prelude::all::EmbedField::new(
                reaction,
                user_mentions,
                false,
            ));
    ...
    if let Err(e) = message
        .channel_id
        .edit_message(&ctx.http, message.id, builder)
        .await
    {
    ...
    async fn test_message_url_format_consistency() {
        ...
        for (_guild_id, expected_guild_part) in guild_cases {
            ...
            let expected_url = format!(
                "https://discord.com/channels/{expected_guild_part}/{channel_id}/{message_id}"
            );
