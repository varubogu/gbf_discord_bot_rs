...
- DB接続:
  - main関数実行～bot起動までの間でコネクションプールを作成し、シングルトンとして動作させる。
  - repository層でDBを使う場合、コネクションプールからコネクションを貸出する。
  - データアクセス層でのみ使用が許可される。
- トランザクション管理:
  - トランザクション抽象オブジェクトはserviceによって取得され、facadeでのみ実際に使用することが許可されます。
  - serviceで取得する際にDB接続を行っておき、データアクセス層以外からはいつDB接続したかを意識せずに済むようにします。
  - トランザクション抽象オブジェクトはserviceを経由してrepositoryに渡され、トランザクションに紐づくコネクションを使って処理を行います。
  - この際、repositoryはトランザクションを引数として受け取るが、DB接続は引数として受け取らないこと
