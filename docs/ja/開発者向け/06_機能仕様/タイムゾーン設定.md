...
- 標準メッセージサービスはDIで構築する（`src/di/message.rs`）
- `src/types/app_state.rs` は `AppMessageService` を保持し、repository具象型を直接importしない
...
**詳細**: [guild_settings.md](../05_データベース/スキーマ/guild_master/guild_settings.md)
