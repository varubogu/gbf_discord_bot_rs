...
**必須環境変数（マイグレーション実行時）**:
- `ADMIN_DB_USER` - Adminロールユーザー名
- `ADMIN_DB_PASSWORD` - Adminロールパスワード

> 補足: 通常起動時も起動シーケンスでマイグレーションを実行するため、通常起動では「共通 + 通常起動のみ + マイグレーション実行時」を全てチェックします。`migrate-only` 実行時は「共通 + マイグレーション実行時」のみチェックします。
...
- 設定ファイルの妥当性チェック（.env.app / .env.db の構文チェック）

## テスト戦略

### 単体テスト

- `EnvValidator::check_common_required_vars()` / `check_bot_startup_required_vars()` / `check_migration_required_vars()` - 環境変数の有無チェック
