...
- コンテキストメニュー「募集を30分遅らせる」による出発時間の後ろ倒し
...
- **募集投稿のDiscord自動削除** - クエスト出発時刻に設定分数を加算した時刻で、元のDiscord募集投稿だけを削除する
...
`/マルチバトル募集` と `/マルチバトル募集2` で作成された募集投稿は、次の時刻にDiscordメッセージ削除タスクを登録します。

```text
quest_start_at + MULTI_RECRUITMENT_DELETE_AFTER_DEPARTURE_MINUTES
```

削除対象は元のDiscord募集投稿のみです。`worker.battle_recruitments` の行、キャンセル通知返信、解散通知返信、その他の永続化データは削除しません。

削除までの猶予は分単位で指定し、次の優先順位で解決します。
...
未設定の場合は次の優先順位へフォールバックします。空文字、不正な数値、0以下の値も次の優先順位へフォールバックし、ユーザー向けメッセージは追加せず警告ログのみ記録します。

削除予定は募集作成時に作成します。出発日時を変更した場合は、その募集に紐づく未実行の削除タスクを削除し、現在の設定で作り直します。環境変数を変更しても、既に作成済みの削除予定は自動再計算しません。募集キャンセル時も削除タスクは削除せず、予定出発時刻 + 解決済み分数で元投稿を削除します。
...
- **キャンセル・変更時の実行者情報解決**: `invoker_user_id`（u64）と `has_bot_control`（bool）をDiscordコンテキストから取り出してFacade層に渡す。`has_bot_control` はHTTP経由で `gbf_bot_control` ロールIDを取得し、実行者のロール一覧と照合して解決する。
...
- 変更・キャンセル結果の集約
- ロール設定の追加・削除処理の統合
- **キャンセル・変更の所有権チェック（ビジネスルール）**: `invoker_user_id` と `has_bot_control` をパラメータで受け取り、「自分の募集 OR 管理者」条件を判定する。このルールはドメインエンティティ（募集メッセージの作成者）に依存するためビジネスロジックとして扱う。
...
- 募集キャンセル時の参加者集計（DB + リアクション）・通知
- 通知ロール取得・メンション文字列生成
- ロール設定の重複チェックと登録・削除

#### Repositoryポート層（repository/）
```
src/repository/
...
```
- 募集データ永続化・通知ロール永続化の契約を定義する
- 募集ボタン/リアクションで利用する参加者永続化契約を定義する
- Serviceから見えるインターフェースをORM非依存に保つ
- `facades/recruitment/**` と `services/recruitment/**` は `crate::repository` で再公開されたtraitにのみ依存する

#### Infrastructureアダプター層（infrastructure/database/repositories/）
```
...
- 具象実装は `src/di/repositories.rs` で生成し、trait互換の依存として注入する
...
- 処理方針:
  ...
  5. 参加者（DB + リアクションを合算し重複除去）にメンションを含む「募集内容が更新されました」通知を送信
  6. DBの募集情報（クエスト、開始日時、バトル種類、テンプレート）を更新
  7. 正常終了時にコミット、失敗時はロールバック

### コンテキストメニュー「募集を30分遅らせる」

- 入力: 対象募集メッセージ（右クリックで選択するのみ。ユーザーの追加入力は不要）
- 実行者: 募集作成者本人、または `gbf_bot_control` ロールを保持する管理者
- 処理概要:
  1. eventsから `postpone_recruitment_departure` を呼ぶ。実体は「募集内容変更」と同じ
     `change_recruitment_information_internal` に `EventDateChange::PostponeMinutes(30)` を渡すだけの薄いラッパー
  2. 変更処理のトランザクション内で既存の募集を読み取った直後、Service層の
     `postpone_quest_departure(現在の出発日時, 分数, 現在時刻)` で新しい出発日時を算出する
     （読み取りと更新が単一トランザクションで完結し、募集レコードの二重読み取りも発生しない）
  3. クエスト・攻略方法は既存値を維持する。参加者への変更通知・出発通知の再作成・投稿削除予定の再作成は
     出発日時変更時と同じ経路で行われる
  4. 変更後の出発日時をギルドのタイムゾーンで整形し、エフェメラルメッセージで実行者に通知
- 異常系:
  - 既に出発時刻を過ぎている募集は後ろ倒ししない（後ろ倒ししても過去のままとなり、出発通知が作成されず
    投稿削除予定だけが過去時刻で再作成されるため）。DBは一切変更せず `RecruitmentChangeOutcome::EventDatePassed` を返す
  - 権限がない場合は募集内容変更と同じ権限エラー文言を返す。募集が見つからない場合はエラー文言を返す

#### 出発日時の変更指定（`EventDateChange`）

`RecruitmentChangeContent.event_date` は「変更しない／指定日時へ変更／相対シフト」を表現する列挙型で、
相対シフトの算出は変更処理のトランザクション内で行う。

| バリアント | 意味 | 利用箇所 |
|---|---|---|
| `Keep` | 出発日時を変更しない | クエスト・攻略方法のみの変更 |
| `Set(DateTime<Utc>)` | 指定日時へ変更 | `/recruit_change`、募集変更パネル |
| `PostponeMinutes(i64)` | 現在の出発日時から指定分だけ後ろへずらす | 「募集を30分遅らせる」 |

変更処理の戻り値は `RecruitmentChangeOutcome` で、適用後の出発日時（`Applied`）か、
後ろ倒し対象外（`EventDatePassed`）かを表す。
...
- 処理概要:
  ...
  4. Service/Facade層でDB参加者とリアクション参加者を合算（重複除去）し、元メッセージに取り消し線と告知を追記
...
- 入力形式: カンマ区切り文字列（上限あり）
- 絶対指定: 時刻のみ / 日付時刻
- 相対指定: 「出発前 n日/時間/分」および同等の英語表現（`後` / `later` / `after` は不許可）
...
- 許容日数は環境変数 `DISMISSAL_MAX_DAYS` で管理（未設定時は `7`）
...
- 日時指定時は指定値をそのまま使用
- 定期募集の解散時刻入力では、日付付き絶対指定は不許可（時刻のみ許可）
