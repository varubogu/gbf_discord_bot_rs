...
> **📝 注意: 一部は推奨パターンです**
>
> このドキュメントに記載されている `InputValidator`, `AllowedValues`, `MessageSanitizer` などの構造体は、
> **推奨パターン**として示されていますが、現時点では実装されていません。
>
> **実際の実装:**
> - 入力検証は各コマンドハンドラで個別に実装
> - SeaORM による SQL インジェクション対策は実装済み
> - 権限チェックは poise の機能と個別実装で対応
>
> これらの推奨パターンを将来的に統合することで、より一貫性のあるセキュリティ実装が可能になります。
