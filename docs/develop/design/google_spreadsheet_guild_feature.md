...
Googleスプレッドシートに記載した設定データをBotの動作に反映させる機能です。**この機能はサーバーごとのデータ読み込み**を行い、スプレッドシートからデータを読み込み、データベースに保存することで、Botの設定やマスターデータを動的に更新できます。

> **注意**: グローバル（全サーバー共通）のスプレッドシート機能については、別途`google_spreadsheet_global_feature.md`を参照してください。

## 機能要件

### 基本機能
- **サーバー固有のスプレッドシート**からのデータ読み込み
- データベースへの一括保存（サーバー固有データ）
- テーブル定義に基づくデータ変換
- エラーハンドリングとログ出力
...
### 対応データ種別（サーバー固有）
1. **クエスト情報**: クエスト名、別名、デフォルトバトル種類（サーバー固有）
2. **イベントスケジュール**: イベント種類、開催回数、期間（サーバー固有）
3. **イベントスケジュール詳細**: 通知メッセージ、スケジュール時間（サーバー固有）
4. **メッセージテキスト**: 多言語対応メッセージ（サーバー固有）
5. **環境設定**: Bot動作設定（サーバー固有）

### コマンド（サーバー固有）
- `/gspread_load`: サーバー固有スプレッドシートからデータ読み込み
- `/gspread_push`: データベースからサーバー固有スプレッドシートへ書き込み（将来実装）
...
| テーブル名（日本語） | テーブル名（英語） | 用途 | IO方向 | スコープ |
|-------------------|------------------|------|--------|----------|
| イベントスケジュール | event_schedules | イベントスケジュール | Input | サーバー固有 |
| イベントスケジュール詳細 | event_schedule_details | スケジュール詳細 | Input | サーバー固有 |
| メッセージ | messages | メッセージテキスト | Input | サーバー固有 |
| 環境設定 | environments | Bot設定 | Input | サーバー固有 |
...
sequenceDiagram
    ...
    participant S as ServerSpreadsheet
    participant D as Database

    U->>C: /gspread_load
    C->>F: execute_load()
    F->>L: open_spreadsheet()
    L->>S: サーバー固有スプレッドシート接続
    S-->>L: 接続完了
    F->>L: load_table_data()
    L->>S: サーバー固有シートデータ取得
    S-->>L: データ
...
#[poise::command(
    slash_command,
    name = "gspread_load",
    description = "サーバー固有スプレッドシートからデータ読み込み"
)]
pub async fn gspread_load(
    ctx: PoiseContext<'_>,
) -> Result<()> {
    // サーバー固有権限チェック（gbf_bot_controlロール）
    ...
    match execute_load(&ctx).await {
        Ok(_) => {
            ctx.say("サーバー固有スプレッドシートからデータ読み込み完了").await?;
        }
        Err(e) => {
            error!(error = %e, "サーバー固有スプレッドシート読み込みエラー");
            ctx.say("サーバー固有スプレッドシートからデータ読み込み失敗").await?;
        }
    }
...
### 権限チェック（サーバー固有）
- `gbf_bot_control`ロールの必須（サーバー固有）
- サーバー固有スプレッドシートアクセス権限
- サーバー固有データベース書き込み権限
...
### ログ出力（サーバー固有）
```rust
info!("サーバー固有スプレッドシート読み込みを開始しました");
info!(table_name = %name, "サーバー固有テーブル読み込みが完了しました");
warn!(table_name = %name, "サーバー固有テーブルに無効な列名があります");
error!(error = %e, "サーバー固有スプレッドシート読み込みに失敗しました");
```
