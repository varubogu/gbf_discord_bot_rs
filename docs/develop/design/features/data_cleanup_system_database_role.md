...
> **⚠️ 注意: Cleanupロールは手動作成が必要**
>
> `gbf_bot_cleanup` ロールのマイグレーションは現時点では実装されていません。
> データベースで手動で作成する必要があります。
>
> **実装状況:**
> - 環境変数設定: `.env.maintenance` にユーザー名を設定済み
> - マイグレーション: 未実装（手動作成が必要）
> - 使用箇所: メンテナンスコンテナ（`src/bin/cleanup.rs`）
>
> **ロール作成手順:**
> このドキュメントの「SQL定義」セクションのSQLを実行してください。
